...
namespace EchoApp
{
    ...
    {
        private static long _wsConnectionCount = 0;
            ...
            app.Use(async (context, next) =>
            {
                if (context.WebSockets.IsWebSocketRequest)
                {
                    ...
                    try
                    {
                        ...
                        {
                            long count = Interlocked.Increment(ref _wsConnectionCount);
                            countIncremented = true;
                            if (count % 100 == 0)
                            {
                                Console.WriteLine($"ws conn count = {_wsConnectionCount}");
                            }
                    ...
                    finally
                    {
                        if (countIncremented)
                        {
                            long count = Interlocked.Decrement(ref _wsConnectionCount);
                            if (count % 100 == 0)
                            {
                                Console.WriteLine($"ws conn count = {_wsConnectionCount}");
                            }
